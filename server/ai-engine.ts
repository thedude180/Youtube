...
Respond as JSON:
{
  ...
  const r = await openai.chat.completions.create({ model: "gpt-5", messages: [{ role: "user", content: p }], response_format: { type: "json_object" }, max_completion_tokens: 10000 });
...
Respond as JSON:
{
  ...
  const r = await openai.chat.completions.create({ model: "gpt-5", messages: [{ role: "user", content: p }], response_format: { type: "json_object" }, max_completion_tokens: 6000 });
  const c = r.choices[0]?.message?.content;
  if (!c) throw new Error("No response from AI");
  return JSON.parse(c);
}
...
Respond as JSON:
{
  ...
  const r = await openai.chat.completions.create({ model: "gpt-5", messages: [{ role: "user", content: p }], response_format: { type: "json_object" }, max_completion_tokens: 8000 });
  const c = r.choices[0]?.message?.content;
  if (!c) throw new Error("No response from AI");
  return JSON.parse(c);
}
...
Respond as JSON:
{
  ...
  const r = await openai.chat.completions.create({ model: "gpt-5", messages: [{ role: "user", content: p }], response_format: { type: "json_object" }, max_completion_tokens: 4000 });
  const c = r.choices[0]?.message?.content;
  if (!c) throw new Error("No response from AI");
  return JSON.parse(c);
}
...
Respond as JSON: { "path": [{"milestone": "milestone name", "resources": "specific resources", "duration": "duration", "why": "why this matters for their channel"}], "schedule": "recommended weekly schedule", "quickWins": ["3-5 things they can do this week"] }`;
  const r = await openai.chat.completions.create({ model: "gpt-5", messages: [{ role: "user", content: p }], response_format: { type: "json_object" }, max_completion_tokens: 2000 });
  const c = r.choices[0]?.message?.content;
  if (!c) throw new Error("No response from AI");
  return JSON.parse(c);
}
