...
export async function executePublish(
  ...
  switch (platform) {
    ...
    case "x":
      return postToX(accessToken, formattedContent, metadata);
    default:
...
function splitIntoThread(text: string, maxLen = 280): string[] {
  ...
  for (const tweet of tweets) {
    if (tweet.length <= maxLen) { result.push(tweet); continue; }
    let remaining = tweet;
...
async function postToX(
  ...
  for (let i = 0; i < tweets.length; i++) {
    ...
    try {
      ...
      return { success: false, platform: "x", error: `X network error: ${networkErr?.message || String(networkErr)}` };
    }

    if (!res.ok) {
      const errText = await res.text().catch(() => "");
      if (res.status === 401) {
        return { success: false, platform: "x", error: "X token expired or revoked. Reconnect X in Settings > Channels." };
      }
      if (res.status === 403) {
        return { success: false, platform: "x", error: `X permission denied (403). Ensure your app has tweet.write scope. Detail: ${errText.substring(0, 200)}` };
      }
      if (res.status === 429) {
        return { success: false, platform: "x", error: "X rate limit reached. Will retry automatically later." };
      }
      if (res.status === 400 && errText.includes("duplicate")) {
        return { success: false, platform: "x", error: "X rejected duplicate tweet. Content was not reposted." };
      }
      return { success: false, platform: "x", error: `X API error (${res.status}): ${errText.substring(0, 200)}` };
    }
  ...
  return {
    ...
    platform: "x",
    postId: firstTweetId,
    postUrl: firstTweetId ? `https://x.com/i/web/status/${firstTweetId}` : undefined,
  };
}
