...
async function runStreamPipelineStep(
  ...
  const prompts: Record<string, string> = {
    ...
    hashtag_strat: `Cross-platform hashtag strategy for "${sourceTitle}". Context: ${ctx("tags")}. Platform-specific hashtag sets, volume, competition. Return JSON: { platforms: object with youtube/tiktok/instagram/discord keys each having hashtags array, volumeScores: object, competitionLevels: object, recommendations: array }`,
    ...
    repurpose: isVod
      ...
      : `Cross-platform repurpose plan for "${sourceTitle}". Context: ${ctx("analyze")}. Unique posts for YouTube, TikTok, X, Discord. Do NOT generate posts for Twitch, Kick, or Rumble — those are streaming-only platforms with no content posting capability. Return JSON: { posts: array of {platform: string, content: string, hashtags: array, tone: string} }`,
