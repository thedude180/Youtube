...
const logger = createLogger("team-ops");
...
export function registerTeamOpsRoutes(app: Express) {
  ...
  app.get("/api/team-ops/status", async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error("[team-ops] Failed to get company status:", err?.message || err);
  ...
  app.get("/api/team-ops/feed", async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error("[team-ops] Failed to get cross-team feed:", err?.message || err);
      res.json([]);
    }
  });
