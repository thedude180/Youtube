...
export function buildPrompts(videoTitle: string, mode: string, existingResults: Record<string, any>): Record<string, string> {
  ...
  if (mode === "live") {
    return {
      ...
      description: `Write a LIVE STREAM description for: "${sanitizeForPrompt(safeTitle)}" — stream is LIVE RIGHT NOW. Context: ${ctx("analyze")}. Include: urgency-driven first 2 lines ("I'm LIVE right now playing..."), link to stream placeholder, what viewers will see if they join NOW, call-to-action to tune in immediately. Return as JSON with key: description (string), keywords (array), seoScore (number 1-100).`,
  ...
  return {
    ...
    description: `Write an SEO-optimized YouTube description for: "${sanitizeForPrompt(safeTitle)}". Context: ${ctx("analyze")}. Include: compelling first 2 lines (shown in search), timestamps placeholder, relevant keywords, call-to-action, social links placeholder. Return as JSON with key: description (string), keywords (array), seoScore (number 1-100).`,
