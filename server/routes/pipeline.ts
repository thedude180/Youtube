...
export function buildPrompts(videoTitle: string, mode: string, existingResults: Record<string, any>): Record<string, string> {
  ...
  if (mode === "live") {
    return {
      ...
      tags: `Generate tags and hashtags for a LIVE stream: "${videoTitle}". Context: ${ctx("analyze")}. Include: live-specific tags (#live #livestream), content-specific tags, trending live tags. Optimize for ALL platforms: ${allPlatforms}. Return as JSON with keys: tags (array), hashtags (array), trendingTags (array).`,
  ...
  return {
    analyze: `Analyze this video titled "${videoTitle}". Identify: 1) Key compelling moments (highlights, wins, fails, emotional peaks) 2) Main topics 3) Target audience 4) Content category 5) Estimated engagement potential (low/medium/high). Return as JSON with keys: keyMoments (array), mainTopics (array), targetAudience (string), category (string), engagementPotential (string), summary (string).`,
    title: `Generate 5 optimized YouTube title options for a video. Original title: "${videoTitle}". Context: ${ctx("analyze")}. Each title should: use power words, create curiosity, include relevant keywords, be under 60 chars. Return as JSON with key: titles (array of objects with title, hookType, estimatedCTR).`,
