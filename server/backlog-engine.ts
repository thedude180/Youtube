...
export async function viralOptimizeVideo(userId: string, videoId: number): Promise<{
  ...
  // Fast-exit: check the in-memory exhaustion cache FIRST so the 44 semaphore
  // waiters generated by a bulk catalog import all bail here without a DB round-
  // trip or a cap-hit log entry.  The cache is set below on the first rejection.
  if (isViralCapCachedExhausted()) {
    return { optimized: false, youtubeUpdated: false, thumbnailQueued: false, seoScore: 0, error: "viral-optimizer cap exhausted (cached)" };
  }
  ...
  // Also set the exhaustion cache so all other waiters still in the queue bail
  // at the fast-exit at the top of this function without another DB round-trip.
  if (isViralCapCachedExhausted()) {
    _releaseViralOpt();
    return { optimized: false, youtubeUpdated: false, thumbnailQueued: false, seoScore: 0, error: "viral-optimizer cap exhausted (cached, post-acquire)" };
  }
  {
