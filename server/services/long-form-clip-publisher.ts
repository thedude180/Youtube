...
// Capped at 15 min for now — longer buckets require 2-3 GB yt-dlp downloads
// that exceed Replit disk constraints and cause silent pre-encoder failures.
// Expand back to [8, 10, 15, 20, 30, 45, 60] once long-form uploads are
// confirmed healthy on an environment with adequate disk headroom.
const EXPERIMENT_DURATIONS_MIN = [8, 10, 15] as const;
...
function pickExperimentDurationSec(maxAvailableSec: number, existingMin?: number): number {
  if (existingMin && existingMin >= 8 && existingMin <= 15) {
...
// Temp dir for JIT-encoded clips (used when pre-encoder hasn't run yet)
const LONG_FORM_TEMP_DIR = path.join(process.cwd(), "data", "longform-tmp");
if (!fs.existsSync(LONG_FORM_TEMP_DIR)) {
  try { fs.mkdirSync(LONG_FORM_TEMP_DIR, { recursive: true }); } catch { /* non-fatal */ }
}
...
async function jitExtractSegment(sourcePath: string, startSec: number, durationSec: number, outputPath: string): Promise<void> {
  await runCmd("ffmpeg", [
    "-y", "-ss", String(startSec), "-i", sourcePath, "-t", String(durationSec),
...
async function jitDownloadSegment(youtubeId: string, startSec: number, endSec: number, outputPath: string): Promise<void> {
  ...
  const args: string[] = [
    ...
    "-o", outputPath, "--no-playlist", "--quiet", "--no-warnings",
  ];
  ...
  try {
    ...
    for (const item of dueItems) {
      ...
      try {
        ...
        if (!encodedPath) {
          // Pre-encoder hasn't run yet (or file was purged). Attempt JIT encoding
          // inline so items don't stall forever on Replit where the pre-encoder
          // may fail due to disk constraints.
          if (preBuiltPath) {
            await db.update(autopilotQueue)
              .set({ metadata: { ...itemMeta, preEncodedPath: null, preEncodedAt: null } as any })
              .where(eq(autopilotQueue.id, item.id));
            logger.info(`[LongFormPublisher] Pre-encoded file purged for item ${item.id} — falling back to JIT encode`);
          }
          ...
          if (jitYtId && startSec >= 0 && durationSec >= MIN_LONG_FORM_SEC) {
            ...
            try {
              ...
              if (!fs.existsSync(tmpEnc)) throw new Error("FFmpeg produced no output");
              encodedPath = tmpEnc;
              if (fs.existsSync(tmpRaw)) fs.unlinkSync(tmpRaw);
              logger.info(`[LongFormPublisher] JIT encode complete for item ${item.id}`);
            } catch (jitErr: any) {
              if (fs.existsSync(tmpRaw)) fs.unlinkSync(tmpRaw);
              if (fs.existsSync(tmpEnc)) fs.unlinkSync(tmpEnc);
              logger.warn(`[LongFormPublisher] JIT encode failed for item ${item.id}: ${jitErr?.message?.slice(0, 200)}`);
              await db.update(autopilotQueue)
                .set({ status: "scheduled", errorMessage: null })
                .where(eq(autopilotQueue.id, item.id));
              skipped++;
            ...
            logger.info(`[LongFormPublisher] Item ${item.id} not yet pre-encoded and JIT not possible — skipping`);
            await db.update(autopilotQueue)
              .set({ status: "scheduled", errorMessage: null })
              .where(eq(autopilotQueue.id, item.id));
            skipped++;
