...
// ── Migration catalog ─────────────────────────────────────────────────────────
// Human-readable map of every migration: used by GET /api/admin/migrations
// to make the 067-deep chain auditable without reading source code.
//
// category: config | data | schema | cleanup | seeding | security
export const MIGRATION_CATALOG: Record<number, { name: string; description: string; category: string }> = {
   1: { name: "Set Focus Game",                      category: "config",  description: "Sets game_focus:current to 'Battlefield 6'" },
   2: { name: "BF6 Queue Reorder",                   category: "data",    description: "Re-prioritises BF6 content in the autopilot queue" },
   3: { name: "Fix Fake Game Names",                 category: "data",    description: "Corrects 35+ wrong game_name values in back_catalog_videos" },
   4: { name: "Purge Demo Reviewer",                 category: "cleanup", description: "Removes demo reviewer account data" },
   5: { name: "Deduplicate Capability Gaps",         category: "data",    description: "Deduplicates AI capability gap entries" },
   6: { name: "Seed Viral Optimizer Cap",            category: "config",  description: "Seeds viral optimizer hourly AI token cap" },
   7: { name: "Seed Module Hourly Caps",             category: "config",  description: "Seeds per-module hourly AI token caps" },
   8: { name: "Seed All Engine Hourly Caps",         category: "config",  description: "Extends hourly caps to all publishing engines" },
   9: { name: "Schema History Fixes",                category: "schema",  description: "Fixes orphaned rows from schema drift" },
  10: { name: "Purge Bad Video IDs",                 category: "cleanup", description: "Removes queue items with invalid YouTube video IDs" },
  11: { name: "Purge Demo Reviewer Full",            category: "cleanup", description: "Full purge of demo reviewer account including channel rows" },
  12: { name: "Delete Placeholder Channels",         category: "cleanup", description: "Deletes placeholder/test channel rows" },
  13: { name: "Game Name Re-audit",                  category: "data",    description: "Re-audits game names across all back_catalog_videos" },
  14: { name: "Cancel Blocked Source Videos",        category: "cleanup", description: "Cancels queue items blocked by missing source videos" },
  15: { name: "Purge Non-YouTube Items",             category: "cleanup", description: "Permanently fails all non-YouTube scheduled/pending items on boot" },
  16: { name: "Create Error Knowledge Base",         category: "seeding", description: "Seeds system incident log with 30 historical production incidents" },
  17: { name: "Purge Stale YouTube Channels",        category: "cleanup", description: "Deletes stale YouTube channel rows after OAuth reconnect" },
  18: { name: "Recascade Stale YouTube Channels",    category: "data",    description: "Migrates stale channel data to the reconnected channel row" },
  19: { name: "Fail Deadlocked Queue Items",         category: "cleanup", description: "Fails queue items deadlocked by vault download failures" },
  20: { name: "Cancel AI Team Tasks",                category: "cleanup", description: "Cancels AI team task queue items" },
  21: { name: "Fail sWCir3U6m_U",                    category: "cleanup", description: "Hard-fails permanently undownloadable vault entry (failCount=10)" },
  22: { name: "Fail ONGsg4mqxT8 Storm Video",        category: "cleanup", description: "Hard-fails yt-dlp storm video ONGsg4mqxT8 (failCount=10)" },
  23: { name: "Sweep High-FailCount Vault Entries",  category: "cleanup", description: "Sets failCount=10 for all vault entries with ≥3 failures" },
  24: { name: "Fix Vault Sweep updated_at Bug",      category: "schema",  description: "Fixes vault sweep broken by missing updated_at column" },
  25: { name: "Fail Q0pj8SN6WyU",                    category: "cleanup", description: "Hard-fails permanently undownloadable vault entry Q0pj8SN6WyU" },
  26: { name: "Fix permanentFail Status Leak",       category: "schema",  description: "Moves vault entries with permanentFail:true that were stuck as active" },
  27: { name: "Fail Ld07AcKauuI Storm Video",        category: "cleanup", description: "Hard-fails yt-dlp storm video Ld07AcKauuI (failCount=10)" },
  28: { name: "Fail Permanently Inaccessible",       category: "cleanup", description: "Sweeps all HTTP-400-confirmed inaccessible vault entries" },
  29: { name: "Reset Ghost Channel Deferred Clips",  category: "data",    description: "Resets clips deferred to ghost channel 52 back to active" },
  30: { name: "Fail Po4WNli5ZLY",                    category: "cleanup", description: "Hard-fails permanently unavailable vault entry Po4WNli5ZLY" },
  31: { name: "Fail All permanentFail Vault Entries",category: "cleanup", description: "Sweeps ALL permanentFail:true vault entries → status=failed" },
  32: { name: "Fix PS5 Game-Name Fallbacks",         category: "data",    description: "Corrects PS5-Gameplay fallback titles in queue from bad game detection" },
  33: { name: "Reschedule Long-Form June 11",        category: "data",    description: "Redistributes long-form queue items from June 11 over-saturation" },
  34: { name: "Fail MTGcjkK8XQ Storm Video",         category: "cleanup", description: "Hard-fails yt-dlp storm video MTGcjkK8XQ" },
  35: { name: "Stamp Missing permanentFail",         category: "schema",  description: "Stamps permanentFail:true on all failed-status vault entries" },
  36: { name: "Fail OG1And3Dw4 Storm Videos",        category: "cleanup", description: "Hard-fails OG1And3Dw4 storm video group" },
  37: { name: "Seed System Incident Log",            category: "seeding", description: "Seeds 30 historical production incidents into system_incident_log" },
  38: { name: "Delete Channel 52",                   category: "cleanup", description: "Deletes ghost channel 52 (duplicate ET Gaming 274 row)" },
  39: { name: "Reset Hard-Blacklisted Pre-Encoder",  category: "data",    description: "Resets wrongly hard-blacklisted pre-encoder items to allow retry" },
  40: { name: "Cancel Untimestamp Auto-Clips",       category: "cleanup", description: "Cancels auto-clips that have no source YouTube video ID" },
  41: { name: "Fail Smart-Edit Items No VideoId",    category: "cleanup", description: "Fails smart-edit queue items with no source video ID" },
  42: { name: "Backfill Smart-Edit VideoId",         category: "data",    description: "Backfills missing videoId into smart-edit job metadata" },
  43: { name: "Cancel Null-Timestamp Auto-Clips",    category: "cleanup", description: "Cancels auto-clips without startSec/endSec timestamps (→ 0–60s duplicates)" },
  44: { name: "Fix VOD Long-Form Segment Bounds",    category: "data",    description: "Fixes vod-long-form items with null segment bounds" },
  45: { name: "Fail H6egjqm0Xjc Storm Video",        category: "cleanup", description: "Hard-fails yt-dlp storm video H6egjqm0Xjc" },
  46: { name: "Fail Non-YouTube Stream Edit Jobs",   category: "cleanup", description: "Fails stream edit jobs targeting non-YouTube platforms" },
  47: { name: "Strip Non-YouTube Platforms",         category: "data",    description: "Strips non-YouTube entries from stream edit job platform arrays" },
  48: { name: "Hard-Fail Format-Error Pre-Encoder",  category: "cleanup", description: "Hard-fails pre-encoder items with ≥3 DASH/no-format failures" },
  49: { name: "Cancel Blocked Publishing Queue",     category: "cleanup", description: "Cancels publishing items blocked by missing vault entry" },
  50: { name: "Fix Pending Studio Auto-Publish",     category: "data",    description: "Fixes studio_auto_publish items stuck in pending state" },
  51: { name: "Redistribute Schedule",               category: "data",    description: "Redistributes over-saturated publishing schedule slots" },
  52: { name: "BF6-Only Focus Enforce",              category: "config",  description: "Enforces BF6-only focus across all engines and gameFilter settings" },
  53: { name: "Seed BF6 Long-Form Schedule",         category: "seeding", description: "Seeds BF6 long-form publishing schedule slots" },
  54: { name: "Fix Long-Form Schedule",              category: "data",    description: "Fixes long-form schedule after over-saturation event" },
  55: { name: "Blacklist Format-Error Items",        category: "cleanup", description: "Blacklists pre-encoder items with repeated format errors (failCount=3)" },
  56: { name: "Recover False-401 Perm-Skips",        category: "data",    description: "Recovers 283 vault entries wrongly perm-skipped by HTTP 401 auth errors" },
  57: { name: "Log June 2026 Incidents",             category: "seeding", description: "Adds June 2026 production crash incidents to system_incident_log" },
  58: { name: "Blacklist Storm Videos (June)",       category: "cleanup", description: "Hard-fails new storm video IDs discovered in June 2026" },
  59: { name: "Fix File Size BIGINT",                category: "schema",  description: "Migrates file_size INT4→BIGINT on vault/studio/asset tables (2.1 GB overflow)" },
  60: { name: "Purge Non-BF6 Shorts",               category: "cleanup", description: "Purges all non-Battlefield-6 Shorts from the autopilot queue" },
  61: { name: "Purge Non-BF6 Slippage Items",        category: "cleanup", description: "Purges non-BF6 slippage items from the queue" },
  62: { name: "Cancel Non-BF6 Studio Auto-Publish",  category: "cleanup", description: "Cancels non-BF6 items in studio_auto_publish queue" },
  63: { name: "Unlock Multi-Game Catalog",           category: "config",  description: "Enables multi-game catalog import (AC + BF6 both indexed)" },
  64: { name: "Purge Bad Game-Name Items",           category: "cleanup", description: "Purges queue items with bad or undetected game names" },
