# CREATOROS — UNIFIED MASTER BUILD PROMPT v15.0

STATUS: SINGLE-SOURCE BUILD DIRECTIVE
USE: PASTE THIS ONCE INTO REPLIT AGENT. THIS FILE SUPERSEDES SPLIT BASE+PATCH WORKFLOWS.
EDIT RULE: DO NOT FREESTYLE. FOLLOW THIS IN ORDER. APPEND FUTURE CHANGES AS VERSIONED AMENDMENTS, NOT RANDOM REWRITES.
...
## REPLIT EXECUTION DIRECTIVE
When this prompt is pasted into Replit Agent, do not treat it as a brainstorming prompt, product memo, or architecture essay.
Treat it as an execution contract.

You must:
- start immediately with a repo audit against Phase 1
- identify build blockers before adding breadth
- fix the real repo, not an imagined rewrite
- execute in strict phase order
- refuse to skip to a later phase because it sounds more exciting
- stop new feature work whenever a phase gate fails
- keep a live completion map of implemented, partial, scaffolded, mocked, blocked, deferred, and removed systems
- output only real progress, not intent theater

## REPLIT START SEQUENCE
On first run, do exactly this in order:
1. inspect the existing repo structure
2. identify Phase 1 gaps versus this prompt
3. identify overlapping or fake-operational systems to collapse
4. identify the single safest governed workflow to make real first
5. make the minimum file changes required to get that workflow working
6. run typecheck/build/tests for the changed path
7. report results in the required build log format
8. remain in Phase 1 until every Phase 1 gate is actually satisfied

## PHASE GATE ENFORCEMENT
A phase is not complete because the UI exists, the schema exists, or services were named.
A phase is complete only when its required governed workflow, tests, receipts, signals, feature gates, and audit traces are real.

If any current-phase gate fails, the agent must:
- stop adding breadth
...
## NO-SKIP RULE
Do not jump ahead to multistreaming, advanced monetization, live crew, buyer readiness, or full intelligence if the current phase is unstable.
No “future-ready scaffolding” may distort current-phase implementation quality.

## BUILD PASS OUTPUT CONTRACT
After every meaningful execution pass, output exactly these headings in this order:
1. FILES CREATED
2. FILES MODIFIED
3. FILES REMOVED
4. WHAT NOW WORKS
...
## ANTI-STALL RULE
Do not ask for permission to proceed when the next safe step is obvious from the prompt.
Do not output a plan instead of doing the work.
Do not say “I would” when you can actually edit the repo.
Do not promise later execution.
Execute now, report honestly, continue phase-by-phase.
...
CreatorOS should run the full business loop:
...
- add regression gates for content, live, monetization, pacing, attribution, and compliance paths
...
- score registry scaffolding
- source pack registry scaffolding
...
- localization quality and dubbing-rights scaffolding
