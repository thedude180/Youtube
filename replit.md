...
- **Legal & Tax AI Team**: Full page at `/legal-tax` — 9 world-class AI legal advisors (Copyright, Contracts, DMCA, Corporate, Privacy, Employment, Defamation, Music Licensing, International) + 9 world-class AI tax advisors (Self-Employment, Deductions, Business Structure, Creator Income, International Tax, Crypto, State/Local, Retirement, Audit Defense). Each advisor has a specialized system prompt and chat UI. Backend route: `POST /api/legal-tax/chat`, `GET /api/legal-tax/advisors`. Sidebar nav item added (Scale icon). All 12 locale files updated with `legalTax` key.
