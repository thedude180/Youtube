...
-   **Core Engines**:
    ...
    -   **YouTube Quota Manager & Push Backlog**: `server/services/youtube-quota-tracker.ts` + `server/services/youtube-push-backlog.ts` — Tracks daily YouTube API quota usage per user (resets midnight Pacific, 10,000 unit daily limit). Atomic SQL increments prevent race conditions. Smart push-or-queue system: when quota is available, pushes optimizations directly to YouTube; when quota is tight, queues them in a backlog. Backlog auto-processes every 15 minutes via cron. Stale "processing" items auto-recover after 5 minutes. Deduplicates by merging pending updates for the same video. API endpoints: GET /api/youtube/quota, GET /api/youtube/backlog, POST /api/youtube/backlog/process, POST /api/youtube/backlog/retry.
