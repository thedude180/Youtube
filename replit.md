...
- **Auto Agent Orchestrator**: `server/services/agent-orchestrator.ts` — starts per-user background agent sessions on login based on tier. Called from `post-login-init.ts` → `initializeUserSystems`. Tier mapping: free=core_engines_only, youtube/starter=+ai_team(3-4h), pro=+business_agents(6h), ultimate=+all(ai_team 1h, business_agents 4h, legal_tax 6h, team_ops 8h). Session status API: `GET /api/user/agent-session`. Staggered startup delays (20s/40s/60s/90s) prevent server hammering on login.
