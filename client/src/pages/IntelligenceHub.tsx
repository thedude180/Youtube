...
export default function IntelligenceHub() {
  ...
  const { data: mindMapNodes = [] } = useQuery({ queryKey: ["/api/nexus/audience-mind-map"] });
  const { data: anomalies = [] } = useQuery({ queryKey: ["/api/nexus/anomalies"] });
  const { data: sentiment = [] } = useQuery({ queryKey: ["/api/nexus/sentiment-timeline"] });
  const { data: overlaps = [] } = useQuery({ queryKey: ["/api/nexus/audience-overlaps"] });
  const { data: cohorts = [] } = useQuery({ queryKey: ["/api/nexus/cohort"] });
  const { data: platformRanks = [] } = useQuery({ queryKey: ["/api/nexus/platform-priority"] });
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-indigo-300">Deep analytics, audience insights & intelligence engines</p>
          </div>
        </div>
              ...
              <Card className="bg-gray-900/60 border-gray-700/30">
                ...
                <CardContent>
                  <div className="text-center mb-4">
                    <div className="text-6xl font-bold text-white">{creatorScore?.overallScore || 0}</div>
                    <div className="flex items-center justify-center gap-1 mt-1">
                      ...
                      <span className={`text-sm ${creatorScore?.trend === "up" ? "text-green-400" : creatorScore?.trend === "down" ? "text-red-400" : "text-gray-400"}`}>{creatorScore?.trend || "new"}</span>
                    </div>
                  </div>
                ...
                {(anomalies as any[]).length ? (
                  <div className="space-y-3">
                    {(anomalies as any[]).map((a: any) => (
                      <div key={a.id} className={`p-4 rounded-lg border ${a.severity === "high" ? "bg-red-900/20 border-red-500/20" : a.severity === "medium" ? "bg-amber-900/20 border-amber-500/20" : "bg-blue-900/20 border-blue-500/20"}`}>
                        <div className="flex items-center justify-between">
                ...
                {(sentiment as any[]).length ? (
                  <div className="space-y-2">
                    {(sentiment as any[]).map((s: any) => (
                      <div key={s.id} className="flex items-center gap-4 p-3 rounded-lg bg-gray-800/40">
                  ...
                  <div className="text-center py-12 text-gray-400">Sentiment data will appear as your audience engagement grows.</div>
                )}
              </CardContent>
            </Card>
          </TabsContent>
            ...
            <Card className="bg-gray-900/60 border-gray-700/30">
              ...
              <CardContent>
                {(overlaps as any[]).length ? (
                  ...
                  <div className="text-center py-12 text-gray-400">Connect platforms to detect audience overlap with other creators.</div>
                )}
              </CardContent>
            </Card>
          </TabsContent>
                ...
                {(platformRanks as any[]).length ? (
                  <div className="space-y-3">
                    {(platformRanks as any[]).map((r: any, i: number) => (
                      <div key={r.id} className="flex items-center gap-4 p-4 rounded-lg bg-gray-800/40 border border-gray-700/20">
                  ...
                  <div className="text-center py-12 text-gray-400">AI will rank your platforms by ROI and growth potential once data is collected.</div>
                )}
              </CardContent>
            </Card>
          </TabsContent>
