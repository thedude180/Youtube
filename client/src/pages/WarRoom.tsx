    ...
    <div className={`min-h-screen p-6 ${hasActiveCrisis ? "bg-gradient-to-br from-red-950 via-gray-950 to-red-950" : "bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950"}`}>
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-sm ${hasActiveCrisis ? "text-red-300" : "text-gray-400"}`}>{hasActiveCrisis ? `${activeIncidents.length} Active Incident${activeIncidents.length > 1 ? "s" : ""} — Crisis Mode Active` : "All clear — continuous monitoring active"}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className={`border ${hasActiveCrisis ? "bg-red-900/30 border-red-500/20" : "bg-green-900/30 border-green-500/20"}`}>
            <CardContent className="p-4 text-center">
              ...
              <p className="text-lg font-bold text-white">{hasActiveCrisis ? "CRISIS" : "ALL CLEAR"}</p>
          ...
          <Card className="bg-gray-900/60 border-gray-700/30">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-lg font-bold text-white">{activeIncidents.length}</p>
          ...
          <Card className="bg-gray-900/60 border-gray-700/30">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-lg font-bold text-white">{(anomalies as any[]).length}</p>
          ...
          <Card className="bg-gray-900/60 border-gray-700/30">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-lg font-bold text-white">{(incidents as any[]).filter((i: any) => i.status === "resolved").length}</p>
        ...
        {!hasActiveCrisis && (
          <Card className="bg-gray-900/60 border-gray-700/30">
            <CardContent className="py-16 text-center">
              ...
              <h2 className="text-xl font-bold text-green-300">All Systems Normal</h2>
        ...
        {(incidents as any[]).filter((i: any) => i.status === "resolved").length > 0 && (
          <div className="space-y-3">
            ...
            {(incidents as any[]).filter((i: any) => i.status === "resolved").map((incident: any) => (
              <Card key={incident.id} className="bg-gray-900/40 border-gray-700/20 opacity-60">
