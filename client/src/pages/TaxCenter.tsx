...
import { Calculator, TrendingUp, FileText, AlertTriangle, CheckCircle2, Building2 } from "lucide-react";
...
export default function TaxCenter() {
  ...
  const { data: estimates, isLoading } = useQuery<any[]>({
    queryKey: ['/api/tax-estimates', '?year=2026'],
  });

  const { data: revenueSummary } = useQuery<any>({
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-5xl mx-auto">
      <div>
        <h1 data-testid="text-page-title" className="text-2xl font-display font-bold">Tax Center</h1>
        <p data-testid="text-page-subtitle" className="text-sm text-muted-foreground mt-1">AI-powered tax guidance for content creators</p>
      </div>
